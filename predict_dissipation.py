...
def auto_select_keras_file(directory="."):
    """
    Searches the given directory for exactly one *.keras file and returns it.
    If multiple or none are found, returns None or handles it as needed.
    """
    # List .keras files
    ...
    if len(keras_files) == 1:
        # Exactly one file found, return it automatically
        return os.path.join(directory, keras_files[0])
    else:
        # Either 0 or >1 files found; handle logic as needed
        if not keras_files:
...
def main():
    ...
    model_filename = auto_select_keras_file()
    if model_filename is None:
        print("Unable to automatically detect keras file")
